...
if [ -z ${EC2_INSTANCE_COUNT_PER_AZ} ];
then
  EC2_INSTANCE_COUNT_PER_AZ="1"
fi

if [ -z ${AWS_REGION_NAME} ];
then
...
aws s3 cp s3://$POP_STATE_BUCKET/$AWS_REGION_NAME/$PROJECT_ID/$POP_ENV/terraform.tfstate terraform.tfstate
#terraform plan
terraform apply -var="POP_ENV=$POP_ENV" -var="AWS_PROFILE=$AWS_PROFILE" -var="AWS_REGION=$AWS_DEFAULT_REGION" -var="INSTANCE_TYPE=$EC2_INSTANCE_TYPE" -var="INSTANCE_COUNT_PER_AZ=$EC2_INSTANCE_COUNT_PER_AZ" --auto-approve
aws s3 cp terraform.tfstate s3://$POP_STATE_BUCKET/$AWS_REGION_NAME/$PROJECT_ID/$POP_ENV/terraform.tfstate
